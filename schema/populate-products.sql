...
create table features_optimized
  ...
  (jsonb_agg(to_jsonb(features) - 'product_id' - 'id')) as features
  from products left join features
    on (products.id = features.product_id) group by products.id;

  drop table features;

  create index ix_features_optimized on features_optimized (id) using hash;

  create table photos_optimized as select s.style_id, (jsonb_agg(to_jsonb(p) - 'id')) as photos from styles as s left join photos as p on (s.style_id = p.styleId) group by s.style_id;
>>>>>>> main
...
drop table skus;
=======
create table skus_optimized as select s.style_id, (jsonb_agg(to_jsonb(sk) - 'id')) as skus from styles
 as s left join skus as sk on (s.style_id = sk.styleId) group by s.style_id;
